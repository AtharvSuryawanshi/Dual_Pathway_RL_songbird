'''
Only env file for single syllable artificial landscape robustness test.
'''
...
class Environment:
    def __init__(self, seed, parameters, NN):
        ...
        if self.N_SYLL > 1:
            raise ValueError("This script is for single syllable test")
...
def build_and_run(seed, annealing, plot, parameters, NN):
    ...
    return np.mean(rewards[(DAYS-1)*TRIALS-100:(DAYS-1)*TRIALS], axis=0) # only 1 return as benchmark
